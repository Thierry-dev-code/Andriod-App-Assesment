...
class ConsoleMode {
  ...
  Future<void> _saveResults(List<Student> students, String inputPath) async {
    ...
    final defaultPath = _excelService.getOutputPath(inputPath);
    print('Enter the path to save the results Excel file:');
    print('(Press Enter to use default: $defaultPath)');
    stdout.write('> ');
    ...
    if (userInput == null || userInput.isEmpty) {
      ...
      if (!userInput.toLowerCase().endsWith('.xlsx') && !userInput.toLowerCase().endsWith('.xls')) {
        outputPath = '$userInput.xlsx';
      } else {
    ...
    switch (saveResult) {
      case CalculationSuccess<String>(:final message):
        ...
        print('');
        print('-' * 60);
        print('You can now open the file at:');
        print('  $outputPath');
        print('-' * 60);
