...
class _GradeCalculatorScreenState extends State<GradeCalculatorScreen> {
    ...
    switch (result) {
      case CalculationSuccess<List<Student>>(:final data):
        setState(() {
          ...
          _statusMessage = 'Grades calculated successfully! Click "Save Results" to export.';
        });
  ...
  /// Saves the graded results to an Excel file with user-selected location.
  Future<void> _saveResultsToExcel() async {
    if (_gradedStudents == null || _gradedStudents!.isEmpty) {
      setState(() {
        _statusMessage = 'No graded students to save. Please process grades first.';
      });
      return;
    }

    try {
      // Get the directory of the input file for default save location
      final inputDir = _selectedFilePath != null 
          ? path.dirname(_selectedFilePath!) 
          : null;
      
      // Open save file dialog
      final result = await FilePicker.platform.saveFile(
        dialogTitle: 'Save Results Excel File',
        fileName: 'student_grades_results.xlsx',
        type: FileType.custom,
        ...
        setState(() {
          ...
          switch (saveResult) {
            case CalculationSuccess<String>(:final data):
              _outputPath = data;
              _statusMessage = 'Results saved successfully to: ${path.basename(data)}';
            
            case CalculationError<String>(:final errorMessage):
              _statusMessage = 'Save failed: $errorMessage';
  ...
  Widget _buildActionButtons(ColorScheme colorScheme) {
    return Column(
      children: [
        ...
        // Save Results Button - only shown when grades are calculated
        SizedBox(
          width: double.infinity,
          child: ElevatedButton.icon(
            onPressed: _isProcessing || _gradedStudents == null
                ? null
                : _saveResultsToExcel,
            icon: const Icon(Icons.save_alt),
            label: const Text('Save Results to Excel'),
            style: ElevatedButton.styleFrom(
              padding: const EdgeInsets.symmetric(vertical: 16),
              backgroundColor: _gradedStudents != null 
                  ? const Color(0xFF16A34A)  // Green when enabled
                  : colorScheme.surfaceContainerHighest,
              foregroundColor: _gradedStudents != null 
                  ? Colors.white 
